...
### Added
- **多媒体资源并发下载**: 显著提升图片、视频等资源的下载效率
- **LivePhoto 播放**: 支持 LivePhoto 内容的播放
- **文章封面**: 支持文章封面图片的显示
- **失效图检测与清理**: 自动检测并清理失效的图片
- **冗余头像清理**: 清理不再使用的头像文件
- **失效 Post 清理**: 清理已失效的微博内容
- **重新备份**: 支持对缺失图片的微博重新备份
- **本地数据管理**: 添加独立的本地数据管理页面
- **模糊搜索**: 为导出页面添加模糊搜索功能
- **两种导出模板**: 新增支持点击放大的导出模板，保留原有纯静态模板
- **Post ID 显示**: 在界面上显示 Post ID 方便排查问题

### Changed
- **前后端解耦**: 后端初始化失败不再导致闪退，使用默认配置继续运行
- **图片分类显示**: LivePhoto、普通图片、视频封面在界面上清晰区分
- **下载清晰度优先**: 前端请求图片时默认选择分辨率更高的版本
- **日志系统重构**: 使用更规范的 tracing 日志系统
- **页面风格统一**: 调整页面布局和风格，保持一致
- **性能优化**: 优化前端组件渲染性能

### Fixed
- **JSON 精度问题**: 修复过长 ID 因精度问题显示不正确
- **进度条遮挡**: 修复进度条遮挡页面内容的问题
- **下载队列提示**: 修复多媒体资源下载队列为空时显示"完成"易引起误导的问题
